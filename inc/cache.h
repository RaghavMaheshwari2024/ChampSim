...
class CACHE : public MEMORY {
  public:
    ...
    struct PortScheduleEntry {
        ...
        int core_id       = -1;
        int  is_write      = -1;
        PortScheduleEntry(uint64_t s, uint64_t e, int c, int w)
        : start_cycle(s), end_cycle(e), core_id(c), is_write(w) {}
    };
    ...
    bool mockingjay_predictor(uint64_t pc, uint64_t full_addr, uint32_t set, uint32_t cpu, uint8_t type);
